...
async function planSync() {
  ...
  for (const opp of allDeals) {
    ...
    // Non-closed stages require a slug.
    if (!target) {
      ops.push({
        type: 'warning',
    ...
    // Only emit channel pin ops for Pilot/Integration/Active-Won stages.
    // Stages 1/2/4 → Notion-only, leave existing team channels untouched.
...
async function applyOp(op) {
  switch (op.type) {
    ...
    case 'upsert_pin': {
      ...
      const card = formatter.buildStatusCard(opp);
      await slack.upsertPinnedStatusCard(op.channelId, card);
      return;
    }
    case 'close_deal': {
      const opp = await notion.getOpportunityById(op.pageId);
