...
const DB = {
  ...
  ACCOUNTS:      '1ec6e317-85ee-81f2-a4ff-f7001b5e5fa3',
  CONTACTS:      '1ec6e317-85ee-81a1-9982-d46c100517ab',
  NOTES:         '1ec6e317-85ee-8147-b608-c693c77fb060',
};
...
function mapOpportunityPage(page) {
  ...
  return {
    ...
    description:  p[DESCRIPTION_PROP]?.rich_text?.map(t => t.plain_text).join('') || '',
...
async function updateOpportunity(pageId, data) {
  ...
  if (data.slackSummary !== undefined) properties[SLACK_SUMMARY_PROP] = { rich_text: [{ text: { content: (data.slackSummary || '').slice(0, 2000) } }] };
  if (data.description !== undefined)  properties[DESCRIPTION_PROP]   = { rich_text: [{ text: { content: (data.description  || '').slice(0, 2000) } }] };
...
async function createContact(data) {
  ...
  if (data.specialty) properties.Specialty = { rich_text: [{ text: { content: data.specialty } }] };
  if (data.linkedin)  properties.Linkedin  = { url: data.linkedin };
  if (data.status)    properties.Status    = { select: { name: data.status } };
  if (data.type && data.type.length)
    properties.Type = { multi_select: data.type.map(name => ({ name })) };
  if (data.opportunityId)
    properties.Opportunities = { relation: [{ id: data.opportunityId }] };
  if (data.accountId)
    properties.Account = { relation: [{ id: data.accountId }] };
  const page = await withRetry(() => notion.pages.create({
...
async function ensureSchema() {
  ...
  if (!props[DESCRIPTION_PROP])   toAdd[DESCRIPTION_PROP]   = { rich_text: {} };
